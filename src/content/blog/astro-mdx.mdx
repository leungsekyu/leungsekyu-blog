...
2.2 在 `astro.config.ts` 中配置 UnoCSS 集成并重置浏览器样式
...
3.1 使用 `pnpm` 安装 `@iconify-json/logos` 图标集
...
### 用法1：使用 remark 插件 `remark-mermaidjs`（推荐）

#### 1

使用 `pnpm` 安装 `remark-mermaidjs`
...
<Link
  ...
  favicon="/icons/bookmarks/playwright.svg"
/>

#### 3

在 `astro.config.ts` 中注册并自定义 `remark-mermaidjs` remark 插件（自定义 Mermaid 图表使用手绘风格）
...
> 💭 更新1：可能与 Netlify 用户没有权限安装 Playwright Chromium 浏览器相关操作系统依赖项有关，因为在 Koyeb 使用 Docker 部署（可以正常安装相关操作系统依赖项）后显示效果与本地开发环境一致。

<div class="-mt-3">
  <Link
    site="https://leungsekyu.koyeb.app/blog/astro-mdx/"
    name="leungsekyu → 游乐场 | Astro MDX（部署于 Koyeb）"
...
编辑后可以在左下方的 Actions 折叠栏中选择导出 SVG 图片或者直接使用生成的 SVG 图片 URL
...
![](@assets/mermaid-zh-CN-emoji.svg)
...
> ⚠️ 已知问题：Mermaid Live Editor 导出的 SVG 图片在移动端会出现 Emoji 右侧有少许遮挡的情况，原因不明。

### 总结

无论使用哪种用法，均建议使用英文，兼容性最好。
